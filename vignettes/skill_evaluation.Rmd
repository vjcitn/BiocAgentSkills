---
title: "Evaluating Bioconductor Agent Skills"
author:
  - name: Claude code
...
output:
  BiocStyle::html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Evaluating Bioconductor Agent Skills}
...
The `BiocAgentSkills` package ships a collection of **how-to skill documents**
that teach AI agents (Claude Code, GitHub Copilot, Gemini) Bioconductor
conventions and multi-step workflows. A skill document is only as useful as the
code it elicits: if a model ignores the document, uses a deprecated function, or
produces code that produces wrong answers, the skill has failed.
...
2. **Does it produce correct results?** (dynamic execution against ground truth)
...
- Load the `txdbmaker` package (not the deprecated `GenomicFeatures` path)
- Call `makeTxDbFromGFF()` with an explicit `format =` argument
- Extract gene model features with `exonsBy()`, `transcriptsBy()`, and `cdsBy()`
...
The eval folder contains three kinds of files:

| File | Role |
|------|------|
| `ground_truth_bundled.R` | Hard-coded expected counts and coordinates for the txdbmaker bundled GFF3 |
| `ground_truth_yeast.R` | Expected values for S. cerevisiae Ensembl r113 (7 036 genes) |
| `check_txdb.R` | Shared correctness checker: runs a TxDb through a battery of assertions |
| `run_eval_bundled.R` | Full evaluator for the bundled GFF3 (static + dynamic) |
| `run_eval_yeast.R` | Full evaluator for the yeast GFF3 (static + dynamic) |
| `prompts/bundled_prompts.md` | Prompt ladder (P1–P5) for the bundled scenario |
| `prompts/yeast_prompts.md` | Prompt ladder (P1–P6) for the yeast scenario |
| `judge_prompt.md` | Template for Claude-as-judge scoring |
...
`extdata/GFF3_files/a.gff3` — a partial annotation for *Solanum lycopersicum*
(tomato), chromosome SL2.40ch00 only. This scenario requires no network access
and runs quickly (< 30 s on a laptop).

Ground truth (`BUNDLED_FACTS`):

- **488** genes, **488** transcripts, **1 268** exons, **488** CDS groups
- Single seqlevel `SL2.40ch00`
- 4 named genes with exact coordinate and exon-count checks

### Yeast GFF3 (real organism, requires download)

The second scenario uses the *Saccharomyces cerevisiae* Ensembl release 113
GFF3 (R64-1-1 assembly). The file is bundled in the package at
...
- **7 036** genes, **7 036** transcripts, **7 416** exons, **6 600** CDS groups
...
The yeast scenario tests more of the skill's surface area because it requires
the organism name and taxonomy ID metadata arguments.

# Running an Evaluation

## Step 1 — Source the evaluator

```{r source-eval, eval=FALSE}
eval_dir <- system.file("eval1", "eval", package = "BiocAgentSkills")
source(file.path(eval_dir, "run_eval_bundled.R"))
```

This sources `ground_truth_bundled.R` and `check_txdb.R` automatically.

## Step 2 — Evaluate a code string

Pass any R code string (e.g., output from an AI model) to `eval_code_string()`:

```{r eval-string, eval=FALSE}
generated_code <- '
library(txdbmaker)
...
result <- eval_code_string(generated_code)
```

Or evaluate a saved file:

```{r eval-file, eval=FALSE}
...
- **P5 Remote URL**: Asks the model to load directly from the Ensembl FTP URL,
  testing the skill's note that `makeTxDbFromGFF()` accepts remote paths.
- **P6 Adversarial**: Same legacy trap as bundled P5, but with a real file path.

# Static vs Dynamic Checks

## Static checks (code text analysis)

Six checks are applied by regex to the generated code string before execution:
...
Static checks are fast, require no installed packages, and detect the most
common failure modes even when code cannot be executed.

## Dynamic checks (correctness)

The evaluator executes the generated code in a fresh environment, finds any
`TxDb` variable, and passes it to `check_txdb()`. Correctness checks include:
...
- Per-gene coordinate and exon-count assertions for named landmark genes

The yeast evaluator adds per-gene checks for strand, transcript name, and
CDS count for ACT1 (YFL039C).

# LLM-as-Judge Scoring

For qualitative dimensions not captured by execution (conciseness, clarity,
absence of unnecessary steps), `eval/judge_prompt.md` provides a template
for Claude-as-judge scoring.
...
Each dimension is scored PASS (2), PARTIAL (1), or FAIL (0). Maximum score
is 12. Interpretation:
...
1. **Define ground truth** — run the reference implementation and record
   corpus-level counts and landmark-feature coordinates in a `ground_truth_*.R`
   file.
2. **Write a checker** — a function that accepts a result object and a facts
   list, and records PASS/FAIL for each assertion.
3. **Build a prompt ladder** — at minimum: minimal prompt, package-hint prompt,
   skill-aligned prompt, and an adversarial prompt that tries to trigger the most
   common failure mode for the domain.
4. **Assemble an evaluator** — combine static code analysis with dynamic
   execution, following the pattern in `run_eval_bundled.R`.

The two-level structure (static + dynamic) is worth preserving: static checks
catch the most common problems cheaply, while dynamic checks catch subtle
semantic errors that regex cannot.
